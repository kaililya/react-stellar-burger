...
export const setAcceptedOrder = (order:TOrderData):TSetAcceptedOrder<TOrderData> => ({ type: SET_ACCEPTED_ORDER, payload: order });
export const clearAcceptedOrder = ():TClearAcceptedOrder => ({ type: CLEAR_ACCEPTED_ORDER });

type TSetSelectedIngredient<T> = {
...
export const setSelectedIngredient = (ing:TIngredient):TSetSelectedIngredient<TIngredient> => ({ type: SET_SELECTED_INGREDIENT, payload: ing });
